...
int BLEManager::gapEventHandler(struct ble_gap_event *event, void *arg) {
  ...
  case BLE_GAP_EVENT_SUBSCRIBE:
    ESP_LOGI(TAG,
             "Subscribe event: conn_handle=%d attr_handle=%d, prev_notify=%d, "
             "cur_notify=%d",
  ...
  case BLE_GAP_EVENT_ENC_CHANGE:
    if (event->enc_change.status == 0) {
      ...
      ESP_LOGW(TAG, "Connection encryption failed: %d",
...
void BLEManager::begin() {
  ...
  /* Set device name dynamically using MAC address */
  uint8_t mac[6];
  esp_read_mac(mac, ESP_MAC_BT);
  char dynamic_name[32];
  snprintf(dynamic_name, sizeof(dynamic_name), "%s-%02X%02X%02X", DEVICE_NAME,
           mac[3], mac[4], mac[5]);
  ble_svc_gap_device_name_set(dynamic_name);
...
bool BLEManager::hasNewCommand() { return commandAvailable; }
...
bool BLEManager::notifyData(const uint8_t* data, size_t length)
{
    if (!connected || connHandle == BLE_HS_CONN_HANDLE_NONE || length == 0) return false;
    ...
    ESP_LOGI(TAG, "Sending notification: connHandle=%d, valHandle=%d, len=%d", connHandle, s_dataValHandle, (int)length);
